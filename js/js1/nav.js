...
function checkNav() {
    if(window.matchMedia('(max-width: 1900px)').matches && window.matchMedia('(min-width: 1431px)').matches) {
        $("nav.opened"). hide();
        $("nav.horizontal"). show();
        $("section").css("height", "calc(100vh - 140px)").css("width", "calc(100vw - 320px)").css("margin-left", "10px");
        ...
        $("nav.opened"). show();
        $("nav.horizontal"). hide();
        $("section").css("height", "calc(100vh - 100px)").css("width", "calc(100vw - 640px)").css("margin-left", "0");
